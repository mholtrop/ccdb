...
bool ccdb::Assignment::MapData(vector<map<string, string> > &mappedData, const vector<string> &data,
...
//______________________________________________________________________________
std::shared_ptr<Variation> ccdb::Assignment::GetVariation() const
{
    return mVariation;
}
...
//______________________________________________________________________________
void ccdb::Assignment::SetVariation(const std::shared_ptr<Variation>& val)
{
    mVariation = val;
}
...
string ccdb::Assignment::VectorToBlob(const vector<string> &values)
{
    ...
    while (iter < values.end())
    {
        ...
        iter++;
        if (iter != values.end())
            result.append(CCDB_DATA_BLOB_DELIMETER); //see CCDB_DATA_BLOB_DELIMETER in CCDBGlobals.h
    }
