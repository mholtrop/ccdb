...
namespace ccdb
{
    ...
    DataProvider::DataProvider(void) :
            ...
            mVariationsAreLoaded(false)
    {
        //Constructor
        mClassName = __func__ + string("::");   /// Used for error reporting
...
//______________________________________________________________________________
    std::shared_ptr<Directory> DataProvider::GetDirectory(const string &path)
    {
...
//______________________________________________________________________________
    void DataProvider::BuildDirectoryDependencies()
    {
        ...
        */
        ...
        for (auto dirIter = mDirectories.begin(); dirIter < mDirectories.end(); dirIter++)
        {
            // retrieve parent id
            // and check if it have parent directory
            auto &dir = *dirIter; //(*dirIter) cool it is C++, baby
            ...
            if (dir->GetParentId() > 0)
            {
                ...
                {
                    Error(CCDB_ERROR_NO_PARENT_DIRECTORY, mClassName + __func__, "Parent directory with wrong id");
...
//______________________________________________________________________________
    const shared_ptr<Directory> DataProvider::GetRootDirectory()
    {
...
//______________________________________________________________________________
    vector<std::shared_ptr<Directory>>
    DataProvider::SearchDirectories(const string &searchPattern, const string &parentPath/*=""*/, int startWith/*=0*/,
                                    int select/*=0*/ )
    {
...
//______________________________________________________________________________
    ConstantsTypeTable DataProvider::GetConstantsTypeTable(const string &path, bool loadColumns/*=false*/ )
    {
        ...
        //and directory
        const auto &dir = GetDirectory(dirPath.c_str());

        //probably one may wish to check dir to be !=NULL,
        //but such check is in GetConstantsTypeTable(const char* name, Dstd::shared_ptr<Directory>parentDir);
...
//----------------------------------------------------------------------------------------
    ...
    Assignment *DataProvider::GetAssignmentFull(int run, const string &path, const string &variation)
    {
        ...
        if (assigments.size() <= 0) return nullptr;
    ...
    Assignment *
    DataProvider::GetAssignmentFull(int run, const string &path, int version, const string &variation/*= "default"*/)
    {
